...
> **数据全链路顺序：collector 采集 → MySQL 写入 → backend 查询 → frontend 展示（30 秒自动刷新）**
...
然后用记事本打开 `.env`，把 `MYSQL_PASSWORD=123456` 改成你本机 MySQL 的实际密码。
...
启动后访问 http://127.0.0.1:8000 确认看到 `{"message":"微博热搜后端服务运行中"}`。
...
控制台显示"写入 XX 条"后，前端页面会在 **30 秒内自动刷新**并显示最新数据（无需手动刷新）。
...
# 排查 collector 和 backend 是否连接同一个库（新增调试接口）
Invoke-RestMethod http://127.0.0.1:8000/api/debug/db-status
```
...
| 症状 | 排查步骤 |
|------|----------|
| 前端提示"当前暂无数据" | 确认已运行 `python collector\app.py`，查看终端是否打印"写入 XX 条" |
| collector 写入成功但前端不更新 | 访问 `/api/debug/db-status`，确认 `mysql_database` 和 `mysql_host` 与 collector 一致 |
| 后端返回"数据库连接失败" | 检查 `.env` 文件中的 `MYSQL_PASSWORD`，确认 MySQL 服务已启动 |
| 前端报"接口请求失败" | 确认后端已启动，访问 `http://127.0.0.1:8000/` 应返回 JSON |
...
返回字段：`mysql_host`、`mysql_database`、`connected`、`total_records`、`latest_fetch_time`、`latest_5_records`（不含密码）。用于核查 collector 和 backend 是否连接同一个数据库。
